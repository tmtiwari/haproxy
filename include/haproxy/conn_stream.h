...
/* Returns the applect of the appctx from a cs if the endpoint is a
 * connection. Otherwise NULL is returned.
 */
static inline const struct applet *cs_appctx_applet(const struct conn_stream *cs)
{
	const struct appctx *appctx = cs_appctx(cs);

	return (appctx ? appctx->applet : NULL);
}
