...
{
	return ((si->flags & SI_FL_ISBACK) ? si->cs->s->res : si->cs->s->req);
}
...
{
	return ((si->flags & SI_FL_ISBACK) ? si->cs->s->req : si->cs->s->res);
}
...
{
	return ((si->flags & SI_FL_ISBACK) ? si->cs->s->si[0] : si->cs->s->si[1]);
}
